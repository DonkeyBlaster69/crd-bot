...
class Staff(commands.Cog):
    def __init__(self, client):
        ...
        #rawsql
        @client.command(name='staffsql')
        async def staffsql(context,*, command:str=None):
            if command == None:
        ...
        async def on_message(message):
            ...
            if message.channel.id == 656798105957564416:
                if message.content.lower().startswith('[event]') or message.content.lower().startswith('*[event]*') or message.content.lower().startswith('**[event]**') or message.content.lower().startswith('***[event]***'):
                    c.execute("SELECT weekevents FROM stafftable WHERE userid=?", (message.author.id,))
                    c.execute("UPDATE stafftable SET weekevents=? WHERE userid=?", (int(re.sub("[(),\[\]]", "", str(c.fetchall()))) + 1, message.author.id))
                    c.execute("SELECT totalevents FROM stafftable WHERE userid=?", (message.author.id,))
                    c.execute("UPDATE stafftable SET totalevents=? WHERE userid=?", (int(re.sub("[(),\[\]]", "", str(c.fetchall()))) + 1, message.author.id))
                    conn.commit()
        ...
        #reset week events
        @client.command(name='resetweek')
        async def resetweek(context, user:discord.User=0):
        ...
        @client.command(name='clearevents')
        async def clearevents(context):
            ...
            if context.author.id in allowedIDs:
                ...
                try:
                    reply = await client.wait_for('message', check=check, timeout=20)
        ...
        async def events(context, user:discord.Member=None, action:str='view', amount:int=None):
        ...
        @client.command(name='addstaff')
        async def addstaff(context, staff:discord.Member=None):
