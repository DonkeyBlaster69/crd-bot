                ...
                if exitcode == 0:
                    ...
                    while True:
                        if currentmodule == len(modules):
                            ...
                            try:
                                client.unload_extension(modules[currentmodule])
                            except Exception as e:
                                await context.send(f"`{e}` Moving on.")
                            try:
                                client.load_extension(modules[currentmodule])
